...
from .pred import EdgePredModel, BilinEdgePredModel, NodePredModel, FastEdgePredModel, FastNodePredModel
...
class EdgeNodeOnlyModel(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        pred_cfg = self.model_config.get('pred_model', 'fast')
        if pred_cfg == 'basic':
            self.predictor = MetaLayer(EdgePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum), 
                                       NodePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum)
                                      )
        elif pred_cfg == 'bilin':
            self.predictor = MetaLayer(BilinEdgePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum), 
                                       NodePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum)
                                      )
        elif pred_cfg == 'fast':
            self.predictor = MetaLayer(FastEdgePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum), 
                                       FastNodePredModel(self.node_in, self.edge_in, self.leak, bn_momentum=bn_momentum)
                                      )
        else:
